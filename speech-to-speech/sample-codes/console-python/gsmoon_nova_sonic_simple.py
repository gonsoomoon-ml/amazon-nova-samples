...
class SimpleNovaSonic:
    def __init__(self, model_id='amazon.nova-sonic-v1:0', region='us-east-1', debug=False):
        ...
        try:
            while self.is_active:
                ...
                if result.value and result.value.bytes_:
                    ...
                    # # Debug 모드일 때만 JSON을 이쁘게 출력
                    # if self.debug:
                    #     print("\n" + "="*50)
                    #     print("DEBUG: Received JSON Response")
                    #     print("="*50)
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='Nova Sonic Speech-to-Speech Demo')
    parser.add_argument('--debug', action='store_true', help='Enable debug mode with JSON output')
    args = parser.parse_args()

    # Create Nova Sonic client with debug parameter
    nova_client = SimpleNovaSonic(debug=args.debug)
